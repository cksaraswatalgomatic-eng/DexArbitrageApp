...
function applyFailedSearch() {
  ...
  Array.from(failedTblBody.rows).forEach(r => {
...
async function loadData() {
  ...
  try {
    ...
    for (const f of (data.failed||[])) {
      ...
      const link = f.link || '#';
      tr.innerHTML = `<td>${f.time}</td><td><a href="${link}" target="_blank" rel="noopener">${f.hash.slice(0,10)}…${f.hash.slice(-8)}</a></td><td>${f.reason||''}</td>`;
      failedTblBody.appendChild(tr);
    }
    applyFailedSearch();
  } catch (e) {
    summaryEl.innerHTML = `<span class="text-neg">Error: ${e.message}</span>`;
  }
}
